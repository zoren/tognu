...
await $`ssh tognu@linode 'export NVM_DIR="$HOME/.nvm" && . "$NVM_DIR/nvm.sh" && cd app && nvm install && npm install --omit=dev && (pm2 restart tognu --update-env || pm2 start "npm start" --name tognu) && pm2 save'`
